...
| 5 | WebSocket服务器 | WSS/WS模拟认证端点 | ❌ | 协议为加密帧，无法伪造响应 |
| 6 | 抓包服务器 | asyncio TCP捕获端口16000 | ✅ | **成功捕获384字节加密数据** |
| 7 | Cports断连 | 关闭TCP连接迫使重连 | ❌ | 需管理员权限 |
| 8 | 系统代理 | 设置HTTP_PROXY | ❌ | rustls不认系统代理 |
| 9 | pktmon抓包 | 内核级网络捕获 | ⚠️ | 配置不当未生成文件 |
| 10 | netsh trace | Windows内置抓包 | ❌ | 需交互停止 |
...
| # | 方案 | 方法 | 结果 | 原因 |
|---|------|------|------|------|
| 17 | 等长Patch地址 | 192.168.100.145→127.0.0.1 | ⚠️ | 程序正常启动，但主验证IP非硬编码（运行时获取） |
| 18 | DLL Patch | 修改HAP_SDK64.dll | ❌ | DLL不参与验证，从未加载 |
| 19 | 错误码搜索 | 搜索cmp eax,1201 | ❌ | 枚举/跳转表访问 |
| 20 | 应答格式试探 | 修改响应首字节 | ❌ | 无解密密钥 |
